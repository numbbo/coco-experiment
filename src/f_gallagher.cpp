...
#include "transform_obj_norm_by_dim.cpp"
#include "transform_obj_shift.cpp"
#include "transform_vars_blockrotation.cpp"
#include "transform_vars_permutation.cpp"
#include "transform_vars_permutation_helpers.cpp"
#include "transform_vars_scale.cpp"
#include "transform_obj_norm_by_dim.cpp"

#include "transform_vars_conditioning.cpp"
#include "transform_obj_oscillate.cpp"
#include "transform_obj_power.cpp"
#include "transform_obj_penalize.cpp"

#include "f_sphere.cpp"
#include "transform_vars_gallagher_blockrotation.cpp"
/**
...
static coco_problem_t *f_gallagher_bbob_problem_allocate(const size_t function, const size_t dimension,
  ...
  random_numbers =
      coco_allocate_vector(data->number_of_peaks * dimension); /* This is large enough for all cases below */
  ...
  for (i = 1; i < data->number_of_peaks; ++i) {
    ...
    data->peak_values[i] =
        (double)(i - 1) / (double)(data->number_of_peaks - 2) * (fitvalues[1] - fitvalues[0]) + fitvalues[0];
  }
...
f_gallagher_sub_problem_allocate(const size_t number_of_variables) {
  ...
  problem_i->versatile_data =
      (f_gallagher_versatile_data_t *)coco_allocate_memory(sizeof(f_gallagher_versatile_data_t));
  ...
  versatile_data_tmp->sub_problems = nullptr;
  versatile_data_tmp->rotated_x = nullptr;
  versatile_data_tmp->block_size_map = nullptr;
  versatile_data_tmp->first_non_zero_map = nullptr;
  versatile_data_tmp->block_sizes = nullptr;
  versatile_data_tmp->B = nullptr;
...
static double f_gallagher_core(const double *x, size_t number_of_variables,
  ...
  for (i = 0; i < versatile_data->number_of_peaks; i++) {
    ...
    y = w_i *
        exp(-1.0 / (2.0 * ((double)number_of_variables)) * y); /* Wassim: problem_i->evaluate_function is the sphere on
                                                                  a transformed coordiante system with conditioning */
    if (i == 0 || maxf < y) {
      maxf = y;
    }
  }
...
static coco_problem_t *f_gallagher_permblockdiag_bbob_problem_allocate(const size_t function, const size_t dimension,
  ...
  rotation_problem = coco_problem_allocate_from_scalars(
      "dummy rotation", nullptr, nullptr, dimension, -5.0, 5.0, 0.0);
  ...
  P_alpha_i = coco_allocate_vector_size_t(
      number_of_peaks - 1); /* random permutation of the alpha_i's to allow sampling without replacement*/
