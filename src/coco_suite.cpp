...
#include "suite_bbob_noisy.cpp"
#include "suite_bbob.cpp"
#include "suite_bbob_mixint.cpp"
#include "suite_biobj.cpp"
#include "suite_biobj_mixint.cpp"
#include "suite_cons_bbob.cpp"
#include "suite_largescale.cpp"
#include "suite_sbox_cost.cpp"
#include "suite_toy.cpp"
 ...
 * don't correspond to a problem because of suite filtering, it returns nullptr.
 *
 * @note This function needs to be updated when a new suite is added to COCO.
 */
...
static coco_suite_t* coco_suite_allocate(char const* suite_name, size_t const number_of_functions,
  ...
  for (i = 0; i < suite->number_of_functions; i++) {
    ...
    if (strcmp(suite->suite_name, "bbob-noisy") == 0)
      suite->functions[i] = suite->functions[i] + 100;
  }
...
coco_suite_get_dimension_from_dimension_index(coco_suite_t const* suite,
  ...
  if (dimension_idx >= suite->number_of_dimensions) {
    coco_error("coco_suite_get_dimension_from_dimension_index(): dimensions index exceeding the number of dimensions "
               "in the suite");
    return 0; /* Never reached*/
  }
...
void coco_suite_free(coco_suite_t* suite) {
  ...
  if (suite) {
    ...
    if (suite->suite_name) {
      ...
      suite->suite_name = nullptr;
    }
    if (suite->dimensions) {
      coco_free_memory(suite->dimensions);
      suite->dimensions = nullptr;
    }
    if (suite->functions) {
      coco_free_memory(suite->functions);
      suite->functions = nullptr;
    }
    if (suite->instances) {
    ...
    if (suite->default_instances) {
      ...
      suite->default_instances = nullptr;
    }
 ...
 * suite_options can be "" or nullptr. In this case, default values are taken
...
//        coco_free_memory(output_valid);
  ...
  if ((suite_options) && (strlen(suite_options) > 0)) {
    ...
    if (coco_options_read_values(suite_options, "function_indices", option_string) > 0) {
      indices = coco_string_parse_ranges(option_string, 1, suite->number_of_functions, "function_indices",
                                         COCO_MAX_INSTANCES);
      if (indices) {
    ...
    if (coco_options_read_values(suite_options, "instance_indices", option_string) > 0) {
      indices = coco_string_parse_ranges(option_string, 1, suite->number_of_instances, "instance_indices",
                                         COCO_MAX_INSTANCES);
      if (indices) {
...
//        coco_free_memory(output_valid);
  ...
  if ((number_of_active_dimensions < 1) || (suite->number_of_dimensions < 1) || (suite->number_of_functions < 1) ||
 ...
 * problem is wrapped with the observer. If the observer is nullptr, the returned
 * problem is unobserved.
 *
 * @param suite The given suite.
 * @param observer The observer used to wrap the problem. If nullptr, the problem
...
coco_problem_t* coco_suite_get_next_problem(coco_suite_t* suite, coco_observer_t *observer) {
  ...
  if (!coco_suite_is_next_instance_found(suite) && !coco_suite_is_next_function_found(suite) &&
      !coco_suite_is_next_dimension_found(suite)) {
