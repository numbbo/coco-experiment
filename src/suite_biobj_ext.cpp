...
static coco_problem_t* suite_biobj_ext_get_problem(coco_suite_t* suite, const size_t function_idx,
  ...
  if (function_idx < 55) {
    /* A "mag.cpp" formula to compute the BBOB function index from the bi-objective function index */
    function1_idx = num_bbob_functions -
                    coco_double_to_size_t(floor(-0.5 + sqrt(0.25 + 2.0 * (double)(55 - function_idx - 1)))) - 1;
    function2_idx = function_idx - (function1_idx * num_bbob_functions) + (function1_idx * (function1_idx + 1)) / 2;

  } else {
    /* New 41 functions extending the bbob-biobj suite,
     * unfortunately, there is not a simple "mag.cpp" formula anymore. */
    if (function_idx == 55) {
      function1_idx = 0;
      function2_idx = 2;
...
static int check_consistency_of_instances(const size_t dimension, size_t function1, size_t instance1, size_t function2,
  ...
  if (norm < apart_enough) {
    coco_debug("suite_biobj_ext_get_new_instance(): The extreme points of %s are too close in the decision space",
               problem->problem_id);
    ...
    for (f1 = 0; (f1 < num_bbob_functions - 1) && !break_search; f1++) {
      ...
      for (f2 = f1 + 1; (f2 < num_bbob_functions) && !break_search; f2++) {
        ...
        for (d = 0; (d < suite->number_of_dimensions) && !break_search; d++) {
          ...
          if (dimension == 0) {
            if (!warning_produced)
              coco_warning("suite_biobj_ext_get_new_instance(): remove filtering of dimensions to get generally "
  ...
  if (data->new_instances) {
    for (i = 0; i < data->max_new_instances; i++) {
      if (data->new_instances[i]) {
        ...
        data->new_instances[i] = nullptr;
      }
    }
  }
  coco_free_memory(data->new_instances);
  data->new_instances = nullptr;
}
