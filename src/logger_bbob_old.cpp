...
#include "coco_string.cpp"
#include "observer_bbob_old.cpp"
...
static const double weight_constraints =
    1e0; /* factor used in logged indicator (f-f*)^+ + sum_i g_i^+ in front of the sum */

/*static const size_t bbob_old_nbpts_nbevals = 20; Wassim: tentative, are now observer options with these default
 * values*/
...
static size_t bbob_old_dimensions_in_current_infoFile[6] = {0, 0, 0,
                                                            0, 0, 0}; /* TODO should use dimensions from the suite */
...
typedef struct {
  ...
  /*char const* alg_name; the alg name, for now, temporarily the same as the path. Wassim: Now in the observer */
 ...
 * x1 | x2...
 was (bbob-new):
    ...
    "x2...\n";
 */
static char const* bbob_old_file_header_str = "%% "
                                              "f evaluations | "
                                              "g evaluations | "
...
static char const* logger_name = "bbob_old";
static char const* data_format = "bbob-new2"; /* back to 5 columns, 5-th column writes single digit constraint values */

/**
 * adds a formated line to a data file
 */
static void logger_bbob_old_write_data(FILE* target_file, size_t number_of_f_evaluations,
   ...
   */
  fprintf(target_file, "%lu %lu %+10.9e %+10.9e ", (unsigned long)number_of_f_evaluations,
          (unsigned long)number_of_cons_evaluations, best_fvalue - best_value, fvalue);
...
static void logger_bbob_old_error_io(FILE* path, int errnum) {
  char const* error_format = "Error opening file: %s\n ";
...
static void logger_bbob_old_open_dataFile(FILE** target_file, char const* path, char const* dataFile_path,
                                          char const* file_extension) {
  char file_path[COCO_PATH_MAX + 2] = {0};
  char relative_filePath[COCO_PATH_MAX + 2] = {0};
  int errnum;
  strncpy(relative_filePath, dataFile_path, COCO_PATH_MAX - strlen(relative_filePath) - 1);
  strncat(relative_filePath, file_extension, COCO_PATH_MAX - strlen(relative_filePath) - 1);
  coco_join_path(file_path, sizeof(file_path), path, relative_filePath, nullptr);
 ...
 * folder_path
 */
static void logger_bbob_old_openIndexFile(logger_bbob_old_data_t *logger, char const* folder_path,
                                          char const* indexFile_prefix, char const* function_id,
                                          char const* dataFile_path, char const* suite_name) {
  ...
  tmp_file = nullptr; /* to check whether the file already exists. Don't want to use target_file */
  ...
  coco_join_path(file_path, sizeof(file_path), folder_path, file_name, nullptr);
  if (*target_file == nullptr) {
    ...
    if ((tmp_file) && (bbob_old_current_dim == logger->number_of_variables) &&
      ...
      *target_file = fopen(file_path, "a+");
      ...
      if (bbob_old_current_dim != logger->number_of_variables) {
        int i, j;
        for (i = 0; i < bbob_old_number_of_dimensions && bbob_old_dimensions_in_current_infoFile[i] != 0 &&
        ...
        if (i < bbob_old_number_of_dimensions && bbob_old_dimensions_in_current_infoFile[i] == 0) {
          ...
          if (i < bbob_old_number_of_dimensions) { /* dimension already present, need to create a new file */
            newLine = 0;
            file_path[strlen(file_path) - strlen(bbob_old_infoFile_firstInstance_char) - 7] =
      ...
      *target_file = fopen(file_path, "a+"); /* in any case, we append */
      ...
      fprintf(*target_file,
              "suite = '%s', funcId = %d, DIM = %lu, Precision = %.3e, algId = '%s', coco_version = '%s', logger = "
              "'%s', data_format = '%s'\n",
              suite_name, (int)strtol(function_id, nullptr, 10), (unsigned long)logger->number_of_variables, pow(10, -8),
              logger->observer->algorithm_name, coco_version, logger_name, data_format);

      fprintf(*target_file, "%%\n");
      strncat(used_dataFile_path, "_i", COCO_PATH_MAX - strlen(used_dataFile_path) - 1);
      strncat(used_dataFile_path, bbob_old_infoFile_firstInstance_char, COCO_PATH_MAX - strlen(used_dataFile_path) - 1);
...
static void logger_bbob_old_initialize(logger_bbob_old_data_t *logger, coco_problem_t* inner_problem) {
  ...
  char indexFile_prefix[10] =
      "bbobexp"; /* TODO (minor): make the prefix bbobexp a parameter that the user can modify */

  assert(logger);
  assert(inner_problem);
  assert(inner_problem->problem_id);
  ...
  coco_join_path(folder_path, sizeof(folder_path), logger->observer->result_folder, dataFile_path, nullptr);
  coco_create_directory(folder_path);
  strncat(dataFile_path, "/bbobexp_f", COCO_PATH_MAX - strlen(dataFile_path) - 1);
  strncat(dataFile_path, tmpc_funId, COCO_PATH_MAX - strlen(dataFile_path) - 1);
  ...
  logger_bbob_old_openIndexFile(logger, logger->observer->result_folder, indexFile_prefix, tmpc_funId, dataFile_path,
  ...
  strncat(dataFile_path, bbob_old_infoFile_firstInstance_char, COCO_PATH_MAX - strlen(dataFile_path) - 1);
...
static void logger_bbob_old_evaluate(coco_problem_t* problem, const double* x, double* y) {
  ...
  if (logger->number_of_evaluations == 1 || (max_fvalue + sum_cons < logger->best_fvalue)) {
    ...
    if (logger->number_of_evaluations == 1 ||
        coco_observer_log_targets_trigger(logger->targets, logger->best_fvalue - logger->optimal_fvalue)) {
      logger_bbob_old_write_data(
          logger->fdata_file, logger->number_of_evaluations, logger->number_of_evaluations_constraints, y_logged,
          logger->best_fvalue, logger->optimal_fvalue, x, problem->number_of_variables,
          problem->number_of_integer_variables, cons, problem->number_of_constraints, logger->log_discrete_as_int);
    }
  }
  ...
  if (coco_observer_evaluations_trigger(logger->evaluations,
    ...
    logger_bbob_old_write_data(
        logger->tdata_file, logger->number_of_evaluations, logger->number_of_evaluations_constraints, y_logged,
        logger->best_fvalue, logger->optimal_fvalue, x, problem->number_of_variables,
        problem->number_of_integer_variables, cons, problem->number_of_constraints, logger->log_discrete_as_int);
...
static void logger_bbob_old_free(void* stuff) {
  ...
  if (logger->index_file) {
    ...
    logger->index_file = nullptr;
  }
  if (logger->fdata_file) {
    fclose(logger->fdata_file);
    logger->fdata_file = nullptr;
  }
  if (logger->tdata_file) {
    ...
    if (!logger->written_last_eval) {
      logger_bbob_old_write_data(logger->tdata_file, logger->number_of_evaluations,
                                 logger->number_of_evaluations_constraints, logger->best_fvalue, logger->best_fvalue,
                                 logger->optimal_fvalue, logger->best_solution, logger->number_of_variables,
                                 logger->number_of_integer_variables, nullptr, 0, logger->log_discrete_as_int);
    }
    fclose(logger->tdata_file);
    logger->tdata_file = nullptr;
  }

  if (logger->rdata_file) {
    fclose(logger->rdata_file);
    logger->rdata_file = nullptr;
  }

  if (logger->best_solution) {
    coco_free_memory(logger->best_solution);
    logger->best_solution = nullptr;
  }

  if (logger->targets) {
    coco_free_memory(logger->targets);
    logger->targets = nullptr;
  }

  if (logger->evaluations) {
    coco_observer_evaluations_free(logger->evaluations);
    logger->evaluations = nullptr;
  }
...
static coco_problem_t* logger_bbob_old(coco_observer_t *observer, coco_problem_t* inner_problem) {
  ...
  logger_data->index_file = nullptr;
  logger_data->fdata_file = nullptr;
  logger_data->tdata_file = nullptr;
  logger_data->rdata_file = nullptr;
  ...
  if (inner_problem->best_value == nullptr) {
