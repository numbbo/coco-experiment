/**
 * @file transform_vars_gallagher_blockrotation.cpp
...
#include "transform_vars_blockrotation_helpers.cpp"
...
static void transform_vars_gallagher_blockrotation_evaluate(coco_problem_t* problem, const double* x, double* y) {
  ...
  for (i = 0; i < inner_problem->number_of_variables; ++i) {
    ...
    for (j = versatile_data->first_non_zero_map[i];
         j < versatile_data->first_non_zero_map[i] + versatile_data->block_size_map[i]; ++j) {
  ...
  coco_evaluate_function(
      ...
      y); /* does not modify the argument of the call since rotated_x will be used later in the sub_problems*/
...
static coco_problem_t* transform_vars_gallagher_blockrotation(coco_problem_t* inner_problem) {
  ...
  problem = coco_problem_transformed_allocate(inner_problem, data, transform_vars_gallagher_blockrotation_free,
