...
static coco_archive_t *coco_archive_allocate(void) {
  ...
  archive->ideal = nullptr; /* To be allocated in coco_archive() */
  archive->nadir = nullptr; /* To be allocated in coco_archive() */
  archive->number_of_objectives = 2;
  archive->is_up_to_date = 0;
  archive->number_of_solutions = 0;
  archive->hypervolume = 0.0;

  archive->current_solution = nullptr;
  archive->extreme1 = nullptr; /* To be set in coco_archive() */
  archive->extreme2 = nullptr; /* To be set in coco_archive() */
...
coco_archive_t *coco_archive(char const* suite_name, const size_t function, const size_t dimension,
  ...
  problem = coco_suite_get_next_problem(suite, nullptr);
  if (! problem) {
    coco_error("coco_archive(): cannot create problem f%02lu_i%02lu_d%02lu in suite '%s'", (unsigned long)function,
               (unsigned long)instance, (unsigned long)dimension, suite_name);
    return nullptr; /* Never reached */
  }
...
int coco_archive_add_solution(coco_archive_t *archive, const double y1, const double y2, char const* text) {
  ...
  node = avl_item_search_right(archive->tree, insert_item, nullptr);
  ...
  if (!update) {
    coco_archive_node_item_free(insert_item, nullptr);
  } else {
    /* Perform tree update */
    while (next_node) {
    ...
    if (! avl_item_insert(archive->tree, insert_item)) {
      coco_archive_node_item_free(insert_item, nullptr);
...
char const* coco_archive_get_next_solution_text(coco_archive_t *archive) {
  ...
  return nullptr; /* This point should never be reached. */
}
