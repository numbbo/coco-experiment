...
class Noisifier:
    ...
    its value both depend _deterministically_ on the input x.
    ...
    need to obey the interface of `noiser.rand`. ``rands[0]`` is by default
    be uniform in [0,1], ``rands[1]`` is used to sample the Gaussian noise,
    ``abs(rands[2])`` is used to sample added noise and ``abs(rands[-1])``
    is used to sample subtracted noise, both of which are by default the
    same generator giving absolute Cauchy numbers scaled by pi/2 such that
    P(number > k) equals about 1/k when k >= 5.
    ...
    def _fnoise(self, x):
        ...
        assert 0 <= r <= 1, r
        i = 4
        for p, sgn, irand in [(self._params['p_add'], 1, 2),
                              (self._params['p_subtract'], -1, -1)]:
            if r < p:
                assert p > 0, (r, p)
                nn = self._params['rands'][irand](x, i)
                if nn < 0:
                    nn *= -1
                return n + sgn * 3.141592653589793 / 2 * nn
                        # self.randn(x, 2) / self.randn(x, 12)
                # was: return n + sgn * 2 * r / self._params[p] / self.rand0(x)
